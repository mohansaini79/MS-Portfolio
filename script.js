...
function openMobileMenu()  {
  ...
  burgerIcon.classList.add('burger-open');
  burgerBtn.setAttribute('aria-expanded','true');
  document.body.style.overflow = 'hidden';
  if (navCenter) navCenter.style.visibility = 'hidden'; /* hide social icons when drawer open */
}
function closeMobileMenu() {
  ...
  if (navCenter) navCenter.style.visibility = '';      /* restore social icons */
}
...
/* ══════════════════════════════════════════════
   RESUME SPLIT-BUTTON DROPDOWNS
══════════════════════════════════════════════ */
(function initResumeDropdowns() {
  const pairs = [
    { caret: 'resume-caret-nav',  dropdown: 'resume-dropdown-nav'  },
    { caret: 'resume-caret-hero', dropdown: 'resume-dropdown-hero' },
  ];

  pairs.forEach(({ caret: caretId, dropdown: ddId }) => {
    ...
    function toggleDd() {
      dd.getAttribute('aria-hidden') === 'false' ? closeDd() : openDd();
    }

    caret.addEventListener('click', (e) => { e.stopPropagation(); toggleDd(); });

    // Close when clicking outside
    document.addEventListener('click', (e) => {
      if (!caret.parentElement.contains(e.target)) closeDd();
    });
    ...
    // Close when a dropdown item is activated
    dd.querySelectorAll('.rd-item').forEach(item => {
